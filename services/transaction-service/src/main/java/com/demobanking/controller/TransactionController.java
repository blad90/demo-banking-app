...
public class TransactionController {
    ...
    public ResponseEntity<Page<TransactionDTO>> getAllAccountsByCustomerIdPageable(
            @PathVariable Long customerId,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size,
            @RequestParam(defaultValue = "transactionDate") String transactionDate) {
        Pageable pageable = PageRequest.of(page, size, Sort.by(transactionDate).descending());
        Page<TransactionDTO> allAccountsByCustomerId = transactionService.findAllByCustomerId(customerId, pageable);
        return new ResponseEntity<>(allAccountsByCustomerId, HttpStatus.OK);
    }
